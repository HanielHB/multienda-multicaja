  ...
  const addToCart = (product) => {
    ...
    if (currentQty >= product.stock) {
      setStockAlertMessage(
        `Stock insuficiente. Solo hay ${product.stock} unidades de "${product.nombre}" (${product.talla ? "Talla " + product.talla : ""}).`,
      );
      ...
      cart.map((item) => {
        if (item.id === id) {
          ...
          if (newQty > item.stock) {
            setStockAlertMessage(
              `Stock insuficiente. Solo hay ${item.stock} unidades de "${item.nombre}".`,
            );
            setShowStockModal(true);
            return item;
          }
  ...
  const handleBarcodeKeyDown = (e) => {
    if (e.key === "Enter" && searchCode.trim().length > 0) {
      ...
      if (product) {
        ...
        setStockAlertMessage(
          `No se encontró un producto con el código: "${searchCode}"`,
        );
        setShowStockModal(true);
        setSearchCode("");
      }
    ...
    try {
      ...
      await html5QrCode.start(
        ...
        (decodedText) => {
          ...
          const product = productos.find(
            (p) =>
              p.codigoBarras === decodedText || p.codigoInterno === decodedText,
          );
          ...
          if (product) {
            ...
            setStockAlertMessage(
              `No se encontró un producto con el código: "${decodedText}"`,
            );
            setShowStockModal(true);
            stopScanner();
          }
  ...
  const handleCobrar = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        alert(
          "Error: " + (errorData.message || "No se pudo registrar la venta"),
        );
      }
    } catch (err) {
      ...
      <header className="lg:hidden bg-gradient-to-r from-cyan-50 to-blue-50 dark:from-gray-900 dark:to-gray-800 border-b border-cyan-100 dark:border-gray-700 px-4 pt-5 pb-3 flex items-center justify-between page-animate gap-2">
        <div className="flex items-center gap-2 flex-shrink-0">
          <Link
            ...
            className="flex items-center justify-center w-8 h-8 rounded-lg text-neutral-gray hover:bg-white/50 dark:hover:bg-gray-700 transition-colors"
          >
            <span className="material-symbols-outlined">arrow_back</span>
          </Link>
        </div>
        <div className="flex items-center gap-1 sm:gap-4 text-xs sm:text-sm min-w-0">
          <span className="font-medium text-gray-600 dark:text-gray-400 hidden sm:inline">
            SUCURSAL:
          </span>
      ...
      <div className="flex-1 flex overflow-hidden relative">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3 lg:gap-4">
            {groupedProducts.map((group, index) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-full aspect-square rounded-xl bg-gray-50 dark:bg-gray-700/50 mb-3 overflow-hidden flex items-center justify-center relative shadow-sm`}
                  >
                    {product.imagen ? (
                      <img
                        src={product.imagen}
                    ...
                    <div
                      className={`w-full h-full flex items-center justify-center text-white ${product.colorClass}`}
                    ...
                    <div
                      className={`absolute top-2 right-2 px-2 py-1 rounded-full text-[10px] font-bold text-white shadow-sm ${totalStock > 10 ? "bg-emerald-500" : totalStock > 0 ? "bg-amber-500" : "bg-red-500"}`}
                    >
        ...
        <aside className="hidden lg:flex w-52 xl:w-64 bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-800 flex-col slide-up flex-shrink-0">
          ...
          <div className="p-3 grid grid-cols-2 gap-2">
            {paymentMethods.map((method) => (
              <button
                ...
                className={`payment-btn p-3 rounded-xl border-2 flex flex-col items-center gap-1 font-medium text-xs transition-all ${
          ...
          <div className="p-3">
            <button
              onClick={() =>
                selectedPayment && cart.length > 0 && setShowPaymentModal(true)
              }
          ...
          <div className="absolute inset-0 z-50 flex items-stretch">
            ...
            <div className="payment-modal w-full sm:w-96 bg-white dark:bg-gray-900 border-l-4 modal-border border-pink-400 flex flex-col shadow-2xl">
              ...
              <div className="flex justify-end p-4">
                <span
                  className={`px-4 py-1.5 rounded-full text-white text-sm font-bold ${selectedPaymentInfo?.color || "bg-pink-500"}`}
                >
              ...
              <div className="flex-1 px-8 py-4 flex flex-col">
                ...
                <div className="relative mb-6">
                  ...
                  <div className="relative">
                    <input
                      type="text"
                      value={
                        selectedCliente ? selectedCliente.nombre : searchCliente
                      }
                      onChange={(e) => {
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span
                      className={`font-bold ${restante > 0 ? "text-red-500" : "text-gray-400"}`}
                    >
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <button
                    ...
                    className={`py-3 rounded-xl font-bold text-base transition-all ${
                      restante <= 0 && cart.length > 0 && !loading
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        Bs. {item.precioUnit.toFixed(2)} c/u
                      </p>
                    </div>
                    <span className="font-bold text-gray-900 dark:text-white">
                      Bs. {(item.precioUnit * item.cantidad).toFixed(2)}
          ...
          <div className="modal-content relative w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="px-8 pb-8">
              ...
              <div className="flex justify-center mb-6">
                <button className="px-6 py-2.5 rounded-xl font-bold text-sm transition-all bg-yellow-400 text-gray-900 shadow-md cursor-default">
                  Efectivo
                </button>
              </div>
                  ...
                  if (monto > 0) {
                    try {
                      ...
                      if (response.ok) {
                        ...
                        setSaldoCaja(
                          parseFloat(data.saldoActual) || saldoCaja + monto,
                        );
                      } else {
      ...
      <footer className="bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 px-2 lg:px-4 py-1.5 lg:py-2.5 page-animate">
        <div className="flex items-center gap-1.5 lg:gap-2 flex-wrap lg:flex-nowrap">
          ...
          <div className="flex items-center gap-1 lg:gap-2 px-2.5 lg:px-4 py-1.5 lg:py-2 rounded-lg bg-blue-50 dark:bg-blue-900/30 border-2 border-blue-200 dark:border-blue-800 ml-auto lg:ml-0">
            <span className="material-symbols-outlined text-blue-500 text-[14px] lg:text-[16px]">
              account_balance_wallet
            </span>
            <span className="text-[10px] lg:text-xs font-medium text-gray-600 dark:text-gray-400">
              En caja:
            </span>
            <span className="text-xs lg:text-sm font-bold text-blue-600 dark:text-blue-400">
              Bs. {saldoCaja.toFixed(2)}
            </span>
          </div>
        </div>
      </footer>
            ...
            <div className="flex flex-col items-center text-center">
              ...
              {efectivoEnCaja && (
                <div className="w-full bg-gray-50 dark:bg-gray-800 rounded-xl p-4 mb-4">
                  {(() => {
                    const diferencia =
                      parseFloat(efectivoEnCaja || 0) - saldoCaja;
                    if (diferencia === 0) {
                      return (
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {selectedProductGroup[0].nombre}{" "}
                  {selectedProductGroup[0].color &&
                    `- ${selectedProductGroup[0].color}`}
                </p>
              </div>
              <button
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-8 max-w-sm w-full scale-in text-center">
            ...
            <p className="text-3xl font-bold text-emerald-500 mb-6">
              Bs. {ventaCompletada.total.toFixed(2)}
            </p>
            <div className="space-y-3">
              <button
                ...
                className="w-full py-3 rounded-xl bg-primary text-white font-bold hover:opacity-90 transition-all"
              >
                Ver Ticket
              </button>
              <button
                onClick={() => setShowVentaExitosaModal(false)}
                className="w-full py-3 rounded-xl border-2 border-gray-300 font-bold hover:bg-gray-50 transition-all"
              >
                Cerrar
              </button>
            </div>
            ...
            <div id="ticket-print" className="p-6 bg-white">
              ...
              <div className="text-center border-b-2 border-dashed border-gray-300 pb-4 mb-4">
                <p className="font-bold capitalize">
                  {ventaCompletada.tipoDocumento}
                </p>
                <p className="text-primary font-bold">
                  {ventaCompletada.numeroDocumento}
                </p>
                <div className="flex justify-between text-xs mt-2">
                  <span>
                    <strong>FECHA:</strong>{" "}
                    {new Date(ventaCompletada.fecha).toLocaleDateString(
                      "es-ES",
                    )}
                  </span>
                  <span>
                    <strong>HORA:</strong>{" "}
                    {new Date(ventaCompletada.fecha).toLocaleTimeString(
                      "es-ES",
                    )}
                  </span>
                </div>
                <p className="text-xs mt-1">
                  <strong>CAJERO:</strong> {ventaCompletada.vendedor}
                </p>
              </div>
              ...
              <div className="border-b-2 border-dashed border-gray-300 pb-4 mb-4">
                ...
                <p className="text-sm">
                  Nro. Documento: {ventaCompletada.clienteDocumento || "-"}
                </p>
              </div>
                ...
                {ventaCompletada.metodoPago === "efectivo" ? (
                  <>
                    <div className="flex justify-between text-xs">
                      <span>EFECTIVO:</span>
                      <span>
                        {ventaCompletada.montoRecibido
                          ? ventaCompletada.montoRecibido.toFixed(2)
                    ...
                    <div className="flex justify-between text-xs">
                      <span>CAMBIO:</span>
                      <span>
                        {ventaCompletada.vuelto
                          ? ventaCompletada.vuelto.toFixed(2)
                          : "0.00"}
                      </span>
                    </div>
                  </>
